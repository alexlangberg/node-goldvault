...
var Db = function (bookshelf, options) {
  ...
  if (obj.options.saveFailedToDisk) {
    obj.fs.ensureDirSync(obj.options.saveFailedToDisk);
  }
};
...
Db.prototype.saveCartToDisk = function (cart) {
  var obj = this;
  return obj.fs.outputJsonAsync(obj.failedDiskName(cart), cart);
};
  ...
  return new BPromise(function (resolve, reject) {
    var pagePromises = R.map(function (result) {
      return obj.transaction(function (t) {
        return obj.insertCartItem(result, t);
      });
    }, cart.results);
    BPromise.all(pagePromises)
      .catch(function (error) {
...
Db.prototype.insertCartItem = function (cartItem, transaction) {
  var obj = this;
  return obj.insertPage(cartItem, transaction)
    .tap(function (page) {
      return BPromise.map(cartItem.gold, function (gold) {
        return obj.insertSentence(gold, page.id, transaction)
          .tap(function (sentence) {
            return BPromise.map(gold.keywords, function (keyword) {
              return obj.insertSentenceWord(keyword, sentence.id, transaction);
            });
          });
      });
    });
